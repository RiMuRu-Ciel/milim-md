...
let handler = async (m, { conn, command }) => {
  if (/^tod$/i.test(command)) {
    conn.send3Button(m.chat, 'Truth or Dare', 'Ciel-Chann & Rimuru-Sama', 'Truth', '.truth', 'Dare', '.dare', 'RANDOM', `${pickRandom(['.dare', '.truth'])}`, m)
  }
  if (/^truth$/i.test(command)) {
    let res = await fetch(global.API('botstyle', '/api/truth', {}, 'apikey'))
    let json = await res.json()
    if (json.result == "") throw json
    conn.send2Button(m.chat, json.result, 'Ciel-Chann & Rimuru-Sama', 'Truth', '.truth', 'Dare', '.dare', m)
