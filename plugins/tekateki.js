...
`.trim()
    conn.tekateki[id] = [
        await conn.sendButton(m.chat, caption, 'Ciel-Chann & Rimuru-Sama', 'Bantuan', `.tete`, m),
        json, poin,
        setTimeout(async () => {
            if (conn.tekateki[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, 'Ciel-Chann & Rimuru-Sama', 'Teka Teki', `.tekateki`, conn.tekateki[id][0])
