let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) return conn.reply(m.chat, `Contoh penggunaan:\n${usedPrefix + command} aku siapa? @6282173064242 kamu ownerku ><`, m, { contextInfo: { mentionedJid: ['6283128734012@s.whatsapp.net'] } })
  let cm = copy(m)
  let who
  if (text.includes('@0')) who = '0@s.whatsapp.net'
  else if (m.isGroup) who = cm.participant = m.mentionedJid[0]
  else who = m.chat
  if (!who) return conn.reply(m.chat, `Contoh penggunaan:\n${usedPrefix + command} aku siapa? @6282173064242 kamu ownerku ><`, m, { contextInfo: { mentionedJid: ['6282173064242@s.whatsapp.net'] } })
  cm.key.fromMe = false
