...
`.trim()
    conn.tebakkota[id] = [
        await conn.sendButton(m.chat, caption, 'Ｍａｄｅ ｂｙ Ｃｉｅｌ-Ｃｈａｎｎ & Ｒｉｍｕｒｕ-Ｓａｍａ ❤', 'Bantuan', '.teko'),
        json, poin,
        setTimeout(async () => {
            if (conn.tebakkota[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, 'Ｍａｄｅ ｂｙ Ｃｉｅｌ-Ｃｈａｎｎ & Ｒｉｍｕｒｕ-Ｓａｍａ ❤', 'Tebak Kota', '.tebakkota', conn.tebakkota[id][0])
