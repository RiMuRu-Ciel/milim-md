...
Bonus: ${poin} XP
    `.trim()
    conn.asahotak[id] = [
        await conn.sendButton(m.chat, caption, 'Ciel-Chann & Rimuru-Sama', 'Bantuan', '.ao', m),
        json, poin,
        setTimeout(async () => {
            if (conn.asahotak[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, 'Ciel-Chann & Rimuru-Sama', 'Asah Otak', '.asahotak', conn.asahotak[id][0])
